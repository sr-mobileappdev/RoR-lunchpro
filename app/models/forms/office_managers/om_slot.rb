class Forms::OfficeManagers::OmSlot
  attr_writer :params
  ...
  def initialize(user, params = {}, office = nil, existing_slot = nil, modifier_id)
    @current_user = user
    @params = params
    @slot = existing_slot
    @errors = []    
  ...
  def delete
    @slot.assign_attributes(:status => 'inactive', :deactivated_by_id => @modifier_id, :deactivated_at => Time.now)
    unless @slot.valid? && @slot.save!
      @errors += @slot.errors.full_messages
      return (@errors.count == 0)
    end
    true
