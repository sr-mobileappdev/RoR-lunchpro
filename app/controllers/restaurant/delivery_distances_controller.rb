class Restaurant::DeliveryDistancesController < ApplicationRestaurantsController
  ...
  def index
    if @xhr
      if modal?
        render :json => { html: (render_to_string partial: 'shared/modals/restaurants/delivery_distance', layout: false, formats: [:html]) }
        return
      else
