...
function LPForms_SelectTimeframe(component) {
  ...
  this.events = {
    //"change .__first" : "firstSelectorChanged",
    //"change .__second" : "secondSelectorChanged",
    // "click [data-submit]" : "submitForm"
  };


  this.firstSelectorChanged = function(e) {
    that = this
    current_position = $(e.currentTarget).find("option:selected").attr("data-position");
    next_position = this.__find(".__second").find("option:selected").attr("data-position");

    if (parseInt(current_position) > parseInt(next_position)) {
      same_element = this.__find(".__second").find("option[data-position="+current_position+"]")

      if (same_element.next() != undefined) {
        same_element = same_element.next()
        same_element = same_element.next()
      }

      this.__find(".__second").val(same_element.val())
    }
  };

  this.secondSelectorChanged = function(e) {
    that = this
    current_position = $(e.currentTarget).find("option:selected").attr("data-position");
    prev_position = this.__find(".__first").find("option:selected").attr("data-position");

    debug(parseInt(prev_position))

    if (parseInt(current_position) < parseInt(prev_position) || parseInt(prev_position) == 0) {
      same_element = this.__find(".__first").find("option[data-position="+current_position+"]")

      if (same_element.prev() != undefined) {
        same_element = same_element.prev()
        same_element = same_element.prev()
      }

      this.__find(".__first").val(same_element.val())
    }
  };
  ...
  this.bind_handlers = function() {
    ...
    debug("Component handlers bound for LPForms_SelectTimeframe (comp__multi_time_select)")
  };
...
var LPForms_SelectTimeframeInit = function() {
  var component_name = "comp__multi_time_select"

  $("."+component_name).each(function (idx) {
    if (!$(this).hasClass("__comp_active")) {
      obj = new LPForms_SelectTimeframe(this)
  ...
  $(document).on( "lp-component.added", function( event, object ) {
    ...
    if ($(object) && $(object).hasClass(component_name)) {
      obj = new LPForms_SelectTimeframe(object)
