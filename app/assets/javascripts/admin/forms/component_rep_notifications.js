...
function LPRepNotifications(component) {
  ...
  this.bind_handlers = function() {
    ...
    debug("Component handlers bound for LPRepNotifications (comp__rep_notifications)")
  };
...
var LPRepNotifications_Initializer = function() {
  var component_name = "comp__rep_notifications";

  $("."+component_name).each(function (idx) {
      obj = new LPRepNotifications(this);
  ...
  $(document).on( "lp-component.added", function( event, object ) {
    ...
    if ($(object) && $(object).hasClass(component_name)) {
      obj = new LPRepNotifications(object);
