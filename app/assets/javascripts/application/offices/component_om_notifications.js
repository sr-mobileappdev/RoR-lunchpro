...
function LPOmNotifications(component) {
  ...
  this.bind_handlers = function() {
    ...
    debug("Component handlers bound for LPOmNotifications (comp__om_notifications)")
  };
...
var LPOmNotifications_Initializer = function() {
  var component_name = "comp__om_notifications";

  $("."+component_name).each(function (idx) {
      obj = new LPOmNotifications(this);
  ...
  $(document).on( "lp-component.added", function( event, object ) {
    ...
    if ($(object) && $(object).hasClass(component_name)) {
      obj = new LPOmNotifications(object);
