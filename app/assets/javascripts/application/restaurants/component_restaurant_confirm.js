...
function LPRestaurantConfirm(component) {
  ...
  this.showConfirm = function(e) {
    ...
    $(".comp__confirm .__popup_content").html("<p class='small'>"+text+"</p><div class='btn-toolbar mt-3 mb-2' role='toolbar' aria-label='Toolbar with button groups'><a class='btn btn-primary btn-sm targ-complete' data-confirm-method='"+form_method+"' href='"+href+"' data-post>Decline</a> <a class='btn btn-secondary btn-sm targ-close ml-3' href=''>Cancel</a></div>")
  ...
  this.bind_handlers = function() {
    ...
    debug("Component handlers bound for LPRestaurantConfirm (comp__confirm)")
  };
...
var LPRestaurantConfirm_Initializer = function() {
  ...
  $("."+component_name).each(function (idx) {
    if (!$(this).hasClass("__comp_active")) {
      obj = new LPRestaurantConfirm(this)
